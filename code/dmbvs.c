...
// *********************************************************************** //
//
// Copyright (C) 2016, Raffaele Argiento and W. Duncan Wadsworth with contributions
// from Michele Guindani
//
...
// of a stochastic search algorithm for variable selection in addition to the
// Gibbs sampler
//
// standard includes
#include <time.h>
...
void update_beta_jj_ss(double **XX, double **JJ, double **loggamma,
                       double *beta_temp, int *inclusion_indicator,
                       double *prop_per_beta, double mu_be[n_cats][n_vars],
                       double sig_be[n_cats][n_vars], double aa_hp, double bb_hp,
                       int jj);
...
// Principal function using stochastic search variable selection
void dmbvs_ss(double **XX, int **YY, double *alpha, double *beta,
              double mu_al[n_cats], double sig_al[n_cats],
              double mu_be[n_cats][n_vars], double sig_be[n_cats][n_vars],
              double aa_hp, double bb_hp, double *prop_per_alpha,
              double *prop_per_beta, const int GG, const int burn,
...
int main(int argc, char *argv[]){
  ...
  int selection_type = atoi(argv[13]);
  ...
  if(selection_type == 0){
    dmbvs_ss(XX, YY, alpha, beta, mu_al, sig_al, mu_be, sig_be, aa_hp, bb_hp,
    ...
    printf("Unrecognized variable selection argument\n");
  }
...
void dmbvs_ss(double **XX, int **YY, double *alpha, double *beta,
  ...
  for(gg = 0 ; gg < GG ; gg++){
    ...
    for(jj = 0 ; jj < n_cats ; jj++){
      ...
      update_beta_jj_ss(XX, JJ, loggamma, beta_temp, inclusion_indicator,
...
// Gibbs variable selection version
void update_beta_jj(double **XX, double **JJ, double **loggamma,
                    double *beta_temp, int *inclusion_indicator,
                    double *prop_per_beta, double mu_be[n_cats][n_vars],
...
// stochastic search variable selection version
void update_beta_jj_ss(double **XX, double **JJ, double **loggamma,
                       double *beta_temp, int *inclusion_indicator,
                       double *prop_per_beta, double mu_be[n_cats][n_vars],
                       double sig_be[n_cats][n_vars], double aa_hp,
